...
export function useMarketplaceVehicles(scope?: 'seller' | 'admin', status?: string) {
...
export function useVerifyRequest() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, action, reason }: { id: string; action: 'verify' | 'cancel'; reason?: string }) =>
...
export function useCreatePaymentIntent() {
  return useMutation({
    mutationFn: (data: { request_id: string; payment_type: 'DEPOSIT' | 'FINAL' }) =>
