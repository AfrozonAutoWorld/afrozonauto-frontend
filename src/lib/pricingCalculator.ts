...
export function calculateLandedCost(
  ...
  const baseShipping = shippingMethod === 'RoRo'
  ...
  const estimatedDeliveryDays = shippingMethod === 'RoRo'
    ? config.timeline_days_roro
    : config.timeline_days_container;

  const depositAmount = totalUsd * 0.30;
...
export const VEHICLE_MAKES = [
  ...
  'Mercedes-Benz', 'Lexus', 'Nissan', 'Hyundai', 'Kia',
  'Mazda', 'Volkswagen', 'Audi', 'Subaru', 'Jeep',
  'Dodge', 'Ram', 'GMC', 'Acura', 'Infiniti',
  'Cadillac', 'Lincoln', 'Volvo', 'Land Rover', 'Porsche',
];
